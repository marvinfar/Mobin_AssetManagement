...
$(document).delegate("#cmdSaveFactorDet", "click", function (e) {
    ...
    if (checkEmpty($('#txtFindProducts')) || $('#lblFindpdID').text() === "0000") {
        ...
        if ($('#txtQuantity').val() > 0) {
            ...
            var q =Number($("#txtQuantity").val()) , Mojoodi = Number($("#lblMojoodi").text());
            if (q>Mojoodi)  {
                $("#txtQuantity").notify("مقدار موجودی کافی نمی باشد", "warn");
                $("#txtQuantity").select();
                return;
            };
            var strText = "<tr data-id='"+Mojoodi+"'>";
            strText += "<td>" + cnt + "</td>";
...
$(document).delegate("#cmdEditDetail", "click", function (e) {
    ...
    if (fdet[1].length > 0) {
        ...
        $('#lblMojoodi').text($('#tFactorDetail .rowSelected').data('id'));
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    strText += "<tr class='row_" +
                        ...
                        data[i].productNameX +
                        "</td>" +
                        "<td style='width: 10%;background-color:cyan' >" + Null2Empty(data[i].number) +
                        "</td>" +
                        "</tr>";
                }
                $("#LoadingStatusProductDetails").html(" ");
...
$(document).delegate("#selOutFactorPayType", "change", function () {
...
$(document).delegate("#cmdSaveFactor", "click", function() {
    ...
    if (l === 0) {
        $.alert({
            ...
            escapeKey: true,
            backgroundDismiss: true,
            content: "لیست ثبت سفارشات خالی می باشد ",
            type: 'orange',
            boxWidth: '15%',
            useBootstrap: false
        });
    ...
    if ($("#selOutFactorPayType").val() === "1" && Number(tp) !== Number(sum)) {
        $("#lblResidualPrice").text(CurrencyFormat(Number(tp) - Number(sum)));
       $.alert({
            title: 'توجه',
            escapeKey: true,
            backgroundDismiss: true,
            content: "مبلغ فاکتور با مجموع اقساط همخوانی ندارد ",
            type: 'orange',
            boxWidth: '15%',
            useBootstrap: false
        });
    ...
    d[7] = $('#selOutFactorPayType').val();
    $.ajax({
        type: "Post",
        url: "/outputFactor/SaveFactorMainInfo",
        data: { 'd': d },
        dataType: "json",
...
function SaveFactorDetail(outFactorId) {
    var d = [];
    d[0] = outFactorId;
        ...
        $('#SetFactorDetailList tr').each(function() {
            ...
            $.ajax({
                type: "Post",
                url: "/outputFactor/SaveFactorDetailInfo",
                data: { 'd': d },
                dataType: "json",
                success: function (result) {
                    SaveFactorPayType(outFactorId);            
                }
            });
        });
}
...
function SaveFactorPayType(outFactorId) {
    var d = [];
    var l = $("#SetPayType").find('tr').length;
    
    if ($("#selOutFactorPayType").val() === "1") {
        if (l > 0) {
            d[0] = outFactorId;
            $('#SetPayType tr').each(function ()  {
                d[1] = $(this).find("td").eq(0).html();//radif
                ...
                $.ajax({
                    type: "Post",
                    url: "/outputFactor/SaveFactorPayList",
                    data: { 'd': d },
                    dataType: "json",
                    success: function (result) {
    ...
    $.notify("اطلاعات ورود با شماره فاکتور " + outFactorId + " ثبت شد", "success");
    //Clear Page
    $(".txtStyle").val("");
...
$(document).on("dblclick", "#tPDFilter tbody ",
    function () {
        ...
        var Mojoodi = $('.rowSelected').find('td').eq(2).text();
        Mojoodi = Mojoodi === "" ? 0 : Mojoodi;
        $("#txtFindProducts").val(pname);
        $("#lblFindpdID").text(pdid);
        $("#lblMojoodi").text(Mojoodi);
        $("#tPDFilter").css("visibility", "collapse");
        $("#txtFindProducts").select();
    });
